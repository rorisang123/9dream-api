...
const getUsers = (request, response) => {
  pool.query('SELECT * FROM campaign', (error, results) => {
    if (error) {
      throw error
    }
...
const getUserById = (request, response) => {
  ...
  pool.query('SELECT * FROM campaign WHERE user_id = $1', [id], (error, results) => {
    if (error) {
      throw error
    }
    response.status(200).json(results.rows)
...
const updateUser = (request, response) => {
  ...
  pool.query(
    'UPDATE user SET name = $1, email = $2 WHERE user_id = $3',
    [name, email, id],
    (error, results) => {
      if (error) {
        throw error
